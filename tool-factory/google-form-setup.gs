 ...
 * 2. このファイルの中身を全部貼り付ける
 * 3. 上の「実行」ボタンを押す（初回は権限の承認が出るので許可）
 * 4. 実行ログに フォームURL と 編集URL が表示される
 *
 * 生成されるもの:
 * - Google Form「Noe ツール発注フォーム — 自動化ヒアリングシート」
 * - 回答が溜まるスプレッドシート「Noe ツール発注 回答」
 *
 * 発注後の流れ:
 * フォームに回答 → Claudeに「Google Formの発注を確認して製造して」と伝える
 * → Claudeが回答シートを読み、実現可否を判定してから製造する
 */

...
- `/tools/daredemo-tsuen-jichitai/` **こども誰でも通園制度 自治体別ナビ**（渋谷・世田谷・練馬・品川・江戸川・足立＋国基準の7区分）。
...
- **決定的な差**：上限が足立10時間〜渋谷64時間と**6.4倍**開いており、横断で比較できる場所が存在しない。
...
- 横展開：収録6区→23区→政令市。データの正本は各区公式ページ（build_daretsu_navi.py の CITIES に出典URLと確認日を保持）。

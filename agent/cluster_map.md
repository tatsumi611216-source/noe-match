...
- 横展開：**東京23区＋政令指定都市20市＋中核市ほか3市（船橋・八王子・姫路）＝計46自治体＋国基準（8/25）**。実査はエージェント22体を並列で走らせた。次は中核市の残り。
- **金額上限型という第3の設計を発見（8/25）**：八王子市は「月の利用時間に制限はありません」として時間の上限を設けず、「月48,000円を上限として補助」という**金額の上限**で運用。葛飾区も同型。時間で区切る／金額で区切る／所得で減免する、の3パターンがある。
- **データの正本は `scripts/_daretsu_data.py`**（ツールと記事が同じモジュールを import する）。エージェントのJSONは `scripts/merge_daretsu_cities.py` で取り込む。転記しない。
- **記事1本目**: `/articles/daredemo-tsuen-ryokin/`「こども誰でも通園制度の料金はいくら？46自治体の実額と減免を調べた」（8/25公開・本文11,989字）。狙う語はサジェスト実測で本命だった「料金」「デメリット」。**「上乗せ」はサジェストなし＝検索されていないので見出しに使わない**。
- **政令市20市の実測（8/25）**：時間を上乗せしていたのは**京都市のみ（国10＋市2＝12時間）**。ほかの18市はすべて国基準の月10時間。福岡市は令和8年度の上限時間が非公表（令和7年度の市政だよりには「月最大40時間」「国の上限の4倍」とあったが一次情報で確認できず）。
  → **時間の上乗せは東京23区にほぼ固有**。政令市は所得別の減免（生活保護0〜300円上限／所得割77,101円未満100〜200円）で負担を下げる設計が主流。**減免の基準額77,101円は全国ほぼ共通**。
- **料金の型（43自治体・目視確定）**：無償20／有料（1時間300円前後）17／施設ごとに設定5／確認できず1（荒川区）。
  **正規表現の自動分類は使えない**——減免の「生活保護世帯は無料」に反応して福岡市・北九州市を無償に、「施設により異なります」に反応して川崎市を施設ごとに誤分類した。分類は人が読んで決める。データの正本は各区公式ページ（build_daretsu_navi.py の CITIES に出典URLと確認日を保持）。
...
### 2026-09-19 追記｜ゲートは「形式」ではなく「語の需要実測」で読む

69本の型別実測（`agent/cluster_analysis_20260919.md`）で、当たり率は制度×自治体ナビ62%・制度×自治体データ60%に対し、
費用シミュレーター17%・事業者横断比較0%だった。**勝敗を分けたのは器具という形式ではなく語**である。

- ゲートに入れる語は、GSCに表示が出ている語かサジェストに実在する語から取る（ツールを先に決めない）
- 判定には GSC の既存表示を併記する。ゲートGOでもGSC表示が無ければ「未実測」と書く
- 頭語と言い換えを最低2通り通す（例: 「平均初婚年齢」GO・月1,520／「初婚年齢 平均」未収録でNO-GO）
- 新設は「ツール1本＋対のデータ記事1本」を1組とし、出荷基準は `agent/AGENT.md`
  「ツールとデータ記事を優先する（2026-09-19）」に従う
- 9/19の打順10組と NO-GO・CHECK の一覧は `agent/strategy_2026Q4.md`

### 2026-09-19 の割付

| 新規資産 | 帰属 | 位置づけ |
|---|---|---|
| `tools/sengyoshufu-wariai` ＋ `articles/sengyoshufu-wariai-data` | **B**（同棲・新生活・生活費） | 公的統計（白書 令和8年版）の年別器具＋全41年表。被リンクは sengyoshufu-seikatsuhi／tomobataraki-shokuji-data／kakeibo-app-fuufu／tools/seikatsuhi-simulator の4本。`tomobataraki-wariai-data` は10/13まで判定ロック中のため未結線（ロック明けに結ぶ） |
...
### N（妊活）の凍結根拠の更新（2026-08-29実測）

凍結理由は「インデックス率0%・A8該当案件なし」だったが、前者は失効した——
**4本すべて索引済みになり、mitocore-kuchikomi は6.8位・表示46/14日**まで来ている。
凍結は継続する（収益手段がない方の理由が生きているため）。ただし今後この帯の
A8案件が出た場合は、インデックスを理由に見送らないこと。

---

## クラスタ別実績の測り方（2026-08-31 新設）

`python scripts/cluster_gsc.py`（`--days 14` で窓を変更）。**このファイルの割付を写した測定器**で、
クラスタ×種別（記事／ツール）のclick・表示を、直近N日と前N日で並べて出す。
手集計をやめた理由は、軸の取り違え事故が2回起きているため（バンク軸で報告してCEOに訂正された）。
**正本はこのファイル。記事を足したら cluster_gsc.py の割付にも足す。**

### 8/31の棚卸しでわかったこと

- 記事は265本（8/16の185本から+80）。うち**48本は番号付きのレガシー**（`article-14` 等）で、
  すでに noindex ＋「移動しました」＋ sitemap 未掲載。**カニバリの心配はない**（確認済み）
- 新規12本を割付: 産後ケア6本→**A**（8/29の方針どおり）、`daredemo-tsuen-yoyaku` /
  `shussan-iryohi-koujo` / `shussan-mushouka`→**制度**
- **帰属未定が3本**（`rikonritsu-data` 離婚率・`shougai-mikonritsu-data` 生涯未婚率・
  `tomobataraki-wariai-data` 共働き割合）。婚姻・世帯の一般統計で「子育て制度」でも
  「アプリ実績データ」でもない。暫定で**統計**として独立表示している。**帰属はCEO判断待ち**
  （表示は7日で4しかないので急がない）

### cluster_audit の偽陽性（未修正・8/31）

`shussan-iryohi-koujo`（出産費用の医療費控除）が、キーワード「医療費」で
子ども医療費クラスタの未接続として検出されている。出産の医療費控除と子ども医療費助成は
別の主題なので、8/29に直した「料金」「産後」と同じ型の偽陽性。
ただし読者の動線としては繋がりうるので、**キーワードを狭めるか結線するかはCEO判断**。

---

## 追加（2026-08-31）｜結婚新生活支援バンク＝B・Cの回収側に置く制度ツール

`tools/kekkon-shinseikatsu-jichitai`（一都三県61自治体）と `articles/kekkon-shinseikatsu-data` を公開した。

- **位置づけ**: これまでの制度バンク（産後ケア・通園・ひとり親・子ども医療費）はすべて集客クラスタ側で、
  収益経路がなかった。結婚新生活支援は**入籍・引越・新居のタイミングに接する制度**なので、
  クラスタB（新生活・生活費）とC（婚活の手段選び）の読者とそのまま重なる。
  制度バンクの型を、初めて回収側のクラスタに置いたことになる。
- **4本足の現状**: ① `shinkon-hojokin`（既存の集客記事）／② 未接続（新居の固定費・光回線・保険は
...
## 追加（2026-09-08）｜区選択ハブ `tools/kosodate-shien-23ku` ＝ 制度（ツール）

既存バンク5本（ひとり親・子ども医療費・病児保育・誰でも通園・産後ケア）を区で串刺しにする回遊ハブ。実査ゼロ。
tool_gate: 「23区 子育て支援 比較」GO（Google推定208/月・器具なし）／「保育園 点数」は1,920/月だが器具3件でNO-GO→本文H2で受け、練馬のみ既存の点数計算へ案内。
結線: バンクのツール5本・記事5本の関連リストへ各1行（TOOL-FAMILY）。cluster_gsc.py の「制度」ツールに追加済み。

...
- 横展開：23区（8/25）＋政令市8市（8/25・エージェント7体を並列で実査）＝計31自治体＋国基準。次は中核市か、政令市の残り（京都・神戸・広島・仙台・新潟・静岡・浜松・堺・岡山・熊本・相模原・北九州）。
- **政令市の実測（8/25）**：横浜・川崎・さいたま・千葉・大阪・名古屋・札幌の**7市はすべて国基準の月10時間で、時間の上乗せなし**。福岡市は令和8年度の上限時間が非公表（令和7年度の市政だよりには「月最大40時間」「国の上限の4倍」とあったが一次情報で確認できず）。
  → **時間の上乗せは東京23区にほぼ固有**。政令市は所得別の減免（生活保護0〜300円上限／所得割77,101円未満100〜200円）で負担を下げる設計が主流。この対比自体がコンテンツになる。データの正本は各区公式ページ（build_daretsu_navi.py の CITIES に出典URLと確認日を保持）。

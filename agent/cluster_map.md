...
- 横展開：**東京23区＋政令指定都市20市＝計43自治体＋国基準を収録（8/25完了）**。実査はエージェント19体を並列で走らせた。次は中核市（62市）か、都道府県庁所在市。
- **データの正本は `scripts/_daretsu_data.py`**（ツールと記事が同じモジュールを import する）。エージェントのJSONは `scripts/merge_daretsu_cities.py` で取り込む。転記しない。
- **記事1本目**: `/articles/daredemo-tsuen-ryokin/`「こども誰でも通園制度の料金はいくら？43自治体の実額と減免を調べた」（8/25公開・本文11,367字）。狙う語はサジェスト実測で本命だった「料金」「デメリット」。**「上乗せ」はサジェストなし＝検索されていないので見出しに使わない**。

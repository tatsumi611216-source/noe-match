...
FIX = {
 ...
 # 2026-09-01 追加（11〜20位の押し上げ）:
 # 「ガルガル期 由来」10.5位・38表示・クリック0、「ガルガル期 名前の由来」8.8位・8表示。
 # 記事は冒頭で由来（動物の母親が「ガルルル」と唸る様子）を説明しているのに、
 # タイトルに「由来」の語が無かった＝語の不一致。記事にある内容だけをタイトルへ出す。
 # 同じ窓で上位にいる with-nenreiso-data と pairs-marriage-data は 8/29 に差し替えた分の
 # 効果を 9/12 に判定するため、今回は触らない（2週間ルール）。
 "articles/garugaru-ki-guide": (
   "ガルガル期の由来と意味｜いつまで続く・原因・夫の対処法【無料セルフ診断】",
   "ガルガル期という名前の由来は、我が子を守る動物の母親が「ガルルル」と唸る様子から。医学の診断名ではなく俗称です。なぜ起こるのか、いつからいつまで続くのか、ホルモン・睡眠・環境の3要因と、夫が今日からできる対処法、受診すべきサインとの見分け方を整理しました。"),
}

...
def units_cost(prices, n):
    """n回ぶんの自己負担。prices は「n回目の単価の並び」で最後の値が以降に繰り返す。
    要素が None（その回の額が非公表）や並び自体が None なら計算しない。"""
    if n <= 0:
        return 0
    if not prices:
        return None
...
free_visit = [c["name"] for c in real
              if c["visit_prices"] and all(p == 0 for p in c["visit_prices"])]
...
FAQ = [
 ("産後ケアの料金はいくらですか？",
  ...
  % (LEAD, CHECKED)),
 ("宿泊型の金額は1泊あたりですか？",
  "自治体によって数え方が分かれます。ひとつ目は「1日あたりいくら」という単価型で、"
  "1泊2日なら2日ぶんが課金されます。ふたつ目は「1泊2日でいくら、以降1日増えるごとにいくら」"
  "という区分型です。みっつ目は自治体としての単価を持たず、施設が決めた額から自治体の"
  "負担額を差し引いた残りが自己負担になる差額型で、この場合は同じ自治体の中でも施設によって"
  "自己負担が数倍変わります。さらに、使う日数によって単価が変わる自治体もあります。"
  "相模原市は宿泊の6日目から単価が倍になり、新潟市は各類型の1回目が無料です。"
  "単価に回数を掛けるだけでは実際の負担額になりません。"
  "本ツールは自治体ごとに「1回目・2回目…の単価」を持たせて積み上げています。"),
 ("減免があれば自己負担は0円になりますか？",
  "なりません。減免の設計は自治体で3通りに分かれます。ひとつ目は非課税世帯・生活保護世帯を"
  "0円にする全額免除型。ふたつ目は半額や定額まで下げる減額型で、北九州市は減免世帯でも"
  "宿泊型1,000円などの負担が残ります。みっつ目は自治体の負担額を上げるだけの方式で、"
  "岡山市はこの型のため、施設の料金次第では減免世帯でも自己負担が残ります。"
  "また、非課税世帯向けとは別に、所得を問わず使える減額枠を持つ自治体もあります"
  "（さいたま市のクーポン5枚、川崎市の5回目までの減免、堺市の5回までの減額枠など）。"
  "「減免あり」の一言で横に並べると実態を取り違えます。"),
 ("住民税非課税世帯は無料になりますか？",

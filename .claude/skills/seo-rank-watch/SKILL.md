---
name: seo-rank-watch
description: noe-match.com（Noe結婚設計室）の検索順位を「測る→見込みクリック×受け皿単価の大きい語を1つ選ぶ→検索意図を調べる→1つ改善→7日（表示不足なら14日）観察→実測で判定」のループで上げていくスキル。週2回（月・木）実行。主力施策ではなく、効いた改善の型を集める実験台として位置づける。「SEO Rank Watch」「順位ウォッチ」「順位改善を回して」「1位を取りに行く」「seo-rank-watch」等の依頼、および定期タスク affiliate-noematch-seo-rank-watch の実行時に使う。
---
...
### 2. 観察期間が終わった改善を判定する
「レビュー期日到来」に出た項目だけを扱う。判定には**施策後だけの7日窓**、表示が足りなければ**施策後だけの14日窓**を使う（28日窓は施策前の日を含むので使わない）。計測器が窓を選んで表示するので、その表示に従う。
- `データ待ち` / `14日窓待ち` … GSCは4〜5日遅れるため、施策後の日だけで窓がそろっていない。**status は observing のまま**触らない。
- `14日窓でも表示不足（判定不能）` … active に戻し、reviews に「判定不能」と書く。
- `判定可・7日窓` / `判定可・14日窓` … 使った窓を reviews の window に書く。表示が十数回程度しかないので、**判定は「方向が見えた」までの意味しか持たない**。報告でも「効いた」と断定しない。
...
計測器の「選定順」表の**上から1つだけ**選ぶ（2026-09-16改定: 1位への近さではなく、得られるものの大きさで並べる）。
- スコア＝「その語が1位になったときの見込みクリック増（28日表示×(1位のCTR−現順位のCTR)）」×「受け皿係数 1＋ページ内CTAの成果単価/5000円」。CTRは一般的な目安値で、予測ではない
- 前回「効果なし」の語でも表に出ていれば対象。その場合は前回と違う種類の改善を使う
- 上位の語を飛ばしてよいのは、除外条件（下記の直近7日の他自動化など）に当たるときと、ギャップが見つからず改善しようがないときだけ。飛ばした理由は報告に書く
- 表に候補が無ければ、`--candidates` の未登録クエリを watchwords.json に登録してから次回に回す（**その回は改善しない**）
- 新しく登録するときは `ctaYen` にページ内アフィリCTAの最高成果単価を入れる（単価は `agent/article_audit.md`・`agent/affiliate_targets.md`・`agent/AGENT.md` の台帳から。CTAが無ければ0）
...
```json
{
  ...
  "actions": [{
    ...
    "gap": "上位と比べて足りなかったもの",
    "done": "実際に行った改善",
    "type": "title | intro | faq | content | internal-link | data のうち使ったもの（複数可）",
...
- 型の集計: improvement-log の判定済み actions を type 別に「方向が見えた／効果なし／判定不能」で数えた表（同じ型で3件以上そろい向きが揃ったら agent/knowledge.md へ昇格を提案）
```
